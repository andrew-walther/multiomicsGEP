...
#          One facet per DGP scenario (default, sparse_synthetic, low_snr,
...
r <- read.csv(IN_CSV)
if (!"scenario" %in% names(r)) r$scenario <- "default"   # tolerate pre-extension CSVs
n_seeds <- length(unique(r$seed))

agg <- aggregate(c_index ~ scenario + strength + arm, data = r,
                 FUN = function(x) c(mean = mean(x), se = sd(x) / sqrt(length(x))))
agg <- do.call(data.frame, agg)
names(agg)[4:5] <- c("mean_c", "se_c")
...
agg$scenario <- factor(agg$scenario,
                       levels = c("default", "sparse_synthetic", "low_snr", "high_K"),
                       labels = c("default (real EBMF templates)", "sparse_synthetic (F sparsity artifact)",
                                  "low_snr (a_shared 12->6)", "high_K (K_shared 4->8)"))
...
p <- ggplot(agg, aes(x = strength, y = mean_c, color = arm, group = arm)) +
  ...
  labs(x = "Prognostic effect strength (beta_shared multiplier)",
       ...
       title = "Joint model vs. two-step baselines across 4 DGP scenarios",
       subtitle = sprintf("Error bars: +/- 1 SE across %d seeds. Dashed line: chance (C=0.5).", n_seeds)) +
  theme_minimal(base_size = 12) +
  theme(legend.position = "bottom")

ggsave(OUT_PNG, p, width = 9.5, height = 7.5, dpi = 150)
cat(sprintf("Saved: %s\n", OUT_PNG))

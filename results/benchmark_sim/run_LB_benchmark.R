...
#               code/preprocess_desurv.R, results/benchmark_sim/run_ssbmf_benchmark.R
# Usage:        Rscript results/benchmark_sim/run_LB_benchmark.R [--quick]
# ============================================================
...
# Source data-loading hub (provides load_pdac_raw, preprocess_merged_cohorts,
# EXTERNAL_COHORTS, PLATFORM_LOG_TRANSFORM, PDAC_DATA_ROOT, generate_synthetic_benchmark_data,
# and fit_supervised_mf_modular via its internal tryCatch source of fit_modular.R).
suppressMessages(source("results/benchmark_sim/run_ssbmf_benchmark.R"))

# fit_modular.R already sourced by hub above (via tryCatch). Source remaining helpers directly.
# Do NOT re-source fit_modular.R here — its runner block requires real_Y when DATA_MODE="real".
...
if (length(results_rows) > 0) {
  ...
  csv_path   <- file.path(OUT_DIR, "LB_benchmark_results.csv")

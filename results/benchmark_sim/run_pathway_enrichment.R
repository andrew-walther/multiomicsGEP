...
d4 <- load_d4_weights()
ALL_PROGRAMS <- 1:7
ACTIVE_PROGRAMS <- d4$survival_active   # {3, 7}
KEPT_PROGRAMS   <- d4$kept_factors      # {3, 5, 6, 7}: survival-active + genomics-only
...
# Section: T5 -- enrichment across all 4 KEPT programs ----
#
# T1 above is scoped to the 2 survival-active programs, matching the 07/15
# report. T5 adds the 2 genomics-only programs (5, 6), whose enrichment was
# already being computed into fgsea_results_all.rds under the old convention
# but filtered out of every reported table when they were labeled "Inactive"
# (DECISIONS.md 2026-08-19). Emitted as a separate file so T1 and the 07/15
# report stay byte-reproducible.

t5 <- fgsea_results[fgsea_results$program %in% KEPT_PROGRAMS & fgsea_results$padj < 0.10, ]
t5$program_class <- vapply(t5$program, function(k) d4$program_class[[as.character(k)]], character(1))
t5 <- t5[order(t5$program, t5$padj),
         c("program", "label", "program_class", "collection", "set", "size",
           "NES", "pval", "padj", "leading_edge")]
write.csv(t5, file.path(OUT_DIR, "T5_enrichment_kept.csv"), row.names = FALSE)

n_hits_kept <- sapply(KEPT_PROGRAMS, function(k) sum(t5$program == k))
if (any(n_hits_kept == 0)) {
  warning(sprintf("run_pathway_enrichment: kept program(s) %s returned ZERO sets at padj<0.10",
                   paste(KEPT_PROGRAMS[n_hits_kept == 0], collapse = ", ")))
}
cat(sprintf("T5: %d enriched sets at padj<0.10 across kept Programs %s (counts: %s)\n",
            nrow(t5), paste(KEPT_PROGRAMS, collapse = ","),
            paste(sprintf("P%d=%d", KEPT_PROGRAMS, n_hits_kept), collapse = " ")))

# Section: C1 -- per-program summary, three-way class ----
...
c1 <- do.call(rbind, lapply(ALL_PROGRAMS, function(k) {
  ...
  data.frame(program = k, label = d4$program_labels[[as.character(k)]],
             program_class = d4$program_class[[as.character(k)]],
...
# Section: ORA -- all 4 kept programs, top-N in {50, 100, 150} ----

background <- d4$gene_names
ora_results <- do.call(rbind, lapply(KEPT_PROGRAMS, function(k) {
...
cat(sprintf("ORA: %d total enriched-set rows across kept Programs %s x N in {50,100,150}\n",
            nrow(ora_results), paste(KEPT_PROGRAMS, collapse = ",")))

# Section: T2 -- top-100 weighted genes, all 4 kept programs ----

t2 <- top_n_genes_table(d4$EF, KEPT_PROGRAMS, d4$program_labels, n = 100)
...
f1_data <- prepare_dotplot_data(fgsea_results, programs = KEPT_PROGRAMS, top_n = 10)
if (nrow(f1_data) > 0) {
...
# F2: running-ES plot for the single top (lowest padj) set per kept program.
for (k in KEPT_PROGRAMS) {
  # NB: t5[t5$program == k, ][1, ] on a zero-row subset returns a 1-row frame
  # of NAs, not a 0-row frame -- is.na(top_set_row$set) is the only reachable
  # branch of this guard, not nrow() (caught in code review).
  top_set_row <- t5[t5$program == k, ][1, ]
...
# F3: gene-weight heatmap over the union of headline leading-edge genes (all 4 kept programs).
f3_genes <- unique(unlist(lapply(strsplit(t5$leading_edge[t5$program %in% KEPT_PROGRAMS], ";"),

...
\begin{corrbox}
\textbf{Items 1 \& 2:} The previous document defined $R^{k}_{ij}$ and $z^{k}_{i}$ without
incorporating posterior expectations, and used inconsistent superscript notation
($k$ vs.\ $-k$). The corrected definitions below evaluate residuals at
posterior means, using the consistent superscript notation $R^{-k}_{ij}$
and $z^{-k}_{i}$ throughout. Note also that $z^{-k}_{i}$ involves $\bm{\beta}$,
not $F$ --- this corrects an inconsistency in the prior document where
...
where the $k$-th residual is $R^{-k}_{ij} = y_{ij}-\sum_{k'\neq k} \overline{l_{ik'}}\,\overline{f_{jk'}}$, with posterior means used so that the resulting coefficients $A_{ik}$ and $B_{ik}$ are well-defined. Similarly, the $k$-th partial working response for the survival term is

\begin{equation}\label{eq:zk}
    z_{i}^{-k} = z_i - \sum_{k'\neq k}\overline{l_{ik'}}\,\overline{\beta_{k'}}
\end{equation}

\textit{Note: the EBMF paper uses $R_{ij}^{k}$ to represent the posterior first moment ($\overline{l_{ik'}}$ and $\overline{f_{jk'}}$) of the residual, but we use it to represent the residual itself (evaluated at posterior means)}.
...
\begin{itemize}
    \item $l_{ik}$ coefficient (B): $\tau_{j}R^{-k}_{ij}E\left[f_{jk}\right] = \tau_{j}R^{-k}_{ij}\overline{f_{jk}}$
...
\begin{itemize}
    \item $l_{ik}$ coefficient (B): $\mathbf{W}_{ii}z^{-k}_{i}E\left[\beta_{k}\right] = \mathbf{W}_{ii}z^{-k}_{i}\overline{\beta_{k}}$
...
\begin{align}
    ...
    \mathbf{B_{ik}} &= \sum_{j}\tau_{j}R^{-k}_{ij}\overline{f_{jk}} + \mathbf{W}_{ii}z^{-k}_{i}\overline{\beta_{k}}\label{eq:B-L}
\end{align}

Then, for the EBNM problem with $EBNM(x,s)$ where $x_i = \frac{B_{ik}}{A_{ik}}$ and $s^{2}_{i} = \frac{1}{A_{ik}}$, we have that

\begin{align}
    x_i &= \frac{\sum_{j}\tau_{j}R^{-k}_{ij}\overline{f_{jk}} + \mathbf{W}_{ii}z^{-k}_{i}\overline{\beta_{k}}}{\sum_{j}\tau_{j}\overline{f^{2}_{jk}} + \mathbf{W}_{ii}\overline{\beta^{2}_{k}}}\label{eq:xi-L}\\
...
\begin{corrbox}
\textbf{Item 2:} The partial residual for the $F$ update uses the same
posterior-mean definition as for the $L$ update (Eq.~\ref{eq:Rk}), with the
consistent $-k$ superscript notation. This ensures $R^{-k}_{ij}$ is evaluated
at posterior means in both the $L$ and $F$ update equations.
\end{corrbox}

\medskip
and isolate the $k$-th factor by defining the residual excluding factor $k$ (evaluated at posterior means) as
\begin{equation}
    R^{-k}_{ij} = y_{ij}-\sum_{k'\neq k}\overline{l_{ik'}}\,\overline{f_{jk'}}
\end{equation}
such that
\begin{equation}
    y_{ij} = \left(\underbrace{y_{ij}-\sum_{k'\neq k}\overline{l_{ik'}}\,\overline{f_{jk'}}}_{R^{-k}_{ij}}\right) - l_{ik}f_{jk} + l_{ik}f_{jk}
...
\begin{equation}
    F\left(q_{f_k}\right) \approx E_{q_l, q_f, q_{\beta}}\left[\sum_{j}\left[-\frac{1}{2}f^{2}_{jk}\left(\underbrace{\sum_{i}\tau_{j}\overline{l^{2}_{ik}}}_{A_{jk}}\right)+f_{jk}\left(\underbrace{\sum_{i}\tau_{j}R^{-k}_{ij}\overline{l_{ik}}}_{B_{jk}}\right)\right]\right]+E_{q_{f_k}}\left[\log \frac{g_{f_k}(f_k)}{q_{f_k}(f_k)}\right].
\end{equation}

Therefore, we have
\begin{align}
    \mathbf{A_{jk}} &= \sum_{i}\tau_{j}\overline{l^{2}_{ik}}\label{eq:A-F}\\
    \mathbf{B_{jk}} &= \sum_{i} \tau_{j}R^{-k}_{ij}\overline{l_{ik}}\label{eq:B-F}
\end{align}

which yield the following inputs for the EBNM problem
\begin{align}
    x_j &= \frac{\mathbf{B_{jk}}}{\mathbf{A_{jk}}} = \frac{\sum_{i} \tau_{j}R^{-k}_{ij}\overline{l_{ik}}}{\sum_{i}\tau_{j}\overline{l^{2}_{ik}}}\label{eq:xj-F}\\
...
\begin{equation}
    z_i^{-k} = z_i - \sum_{k' \neq k} \overline{l_{ik'}} \,\overline{\beta}_{k'}
...
\begin{align}
    ...
    \mathbf{B_k} &= \sum_i \mathbf{W}_{ii} z_i^{-k} \overline{l_{ik}}\label{eq:B-beta}
\end{align}

This yields the inputs for the 1D EBNM problem with a single pseudo-observation for factor $k$:

\begin{align}
    x_k &= \frac{B_k}{A_k} = \frac{\sum_i \mathbf{W}_{ii} z_i^{-k} \overline{l_{ik}}}{\sum_i \mathbf{W}_{ii} \overline{l_{ik}^2}}\label{eq:xk-beta} \\
...
\medskip
\hspace{3em}Compute partial residuals using current posterior means (Eqs.~\ref{eq:Rk}, \ref{eq:zk}):
\[
    R^{-k}_{ij} = y_{ij} - \sum_{k'\neq k}\overline{l_{ik'}}\,\overline{f_{jk'}},
    \qquad
    z_i^{-k} = z_i - \sum_{k'\neq k}\overline{l_{ik'}}\,\overline{\beta_{k'}}
...
\begin{align*}
    ...
    \qquad
    \mathbf{B_{ik}} = \sum_j \hat{\tau}_j\,R^{-k}_{ij}\,\overline{f_{jk}}
                    + \mathbf{W}_{ii}\,z_i^{-k}\,\overline{\beta_k}\\
...
\begin{align*}
    ...
    \qquad
    \mathbf{B_k} = \sum_i \mathbf{W}_{ii}\,z_i^{-k}\,\overline{l_{ik}}\\

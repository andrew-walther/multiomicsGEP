...
\textbf{What this figure presents.} A dot-and-whisker plot of survival coefficients $\beta_k$ for the five synthetic factors. Blue dots show the posterior mean estimate with 95\% credible intervals. Red X marks show the \emph{permutation-aligned} true $\beta_k$ values --- each true value has been matched to the estimated factor it corresponds to (using the loading correlation matrix from the heatmap) and sign-corrected to match the estimated factor's sign convention. The x-axis labels read ``F1 (T4)'', ``F2 (T3)'', etc., where ``F$k$'' is estimated factor $k$ and ``T$j$'' is the matched true factor $j$.

\textbf{How to interpret it.} Because the permutation alignment and sign correction have already been applied, the blue dots (estimates) and red X marks (truth) should sit close to one another at each x-position --- any remaining gap reflects genuine estimation uncertainty, not an ordering artefact. Prognostic factors (true $|\beta| > 0$) show dots near their matching X marks, with credible intervals that exclude zero. The null factor (true $\beta = 0$) is correctly shrunk toward zero by the EBNM prior. The C-index of 0.86 (Supervised) vs.\ 0.83 (PCA) confirms that the recovered survival structure translates into better patient risk stratification.

\textbf{The takeaway.} With the permutation properly applied, Fig.~3 is a clean validation: estimated and true betas align closely at every factor position, and the null-factor coefficient is correctly zeroed. This is the strongest evidence that the model learns the right survival structure from synthetic data.

\begin{itemize}
  \item \sectionlabel{Key line to say:} ``The null factor --- the one with true $\beta = 0$ --- is correctly shrunk to near zero by the EBNM prior. And every prognostic factor has its true value sitting inside the credible interval. That is the model doing exactly what it should.''
  \item \sectionlabel{Likely question:} ``Why are some dots not exactly on the X mark?'' Answer: the remaining gap is legitimate posterior shrinkage --- empirical Bayes compresses effect sizes slightly toward zero, which reduces variance at the cost of a small bias. This is the bias-variance tradeoff built into the EBNM prior, not a model failure.
\end{itemize}
...
\item[\textbf{S11R}] \actioncue{How do you confirm the model recovers the true survival coefficients?}\\
The beta figure applies permutation alignment and sign correction before plotting, so each estimated $\hat{\beta}_k$ (blue dot) sits directly next to the true $\beta$ it was estimated from (red X). Dots and X marks coincide at every factor --- the model gets the right magnitude and direction. The one factor with true $\beta = 0$ is correctly shrunk to near zero, confirming that the EBNM shrinkage is working as intended.

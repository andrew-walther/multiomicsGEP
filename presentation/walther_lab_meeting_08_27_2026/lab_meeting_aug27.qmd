...
**Results** &nbsp;·&nbsp; The $K_\text{init}$ consensus sweep · gene programs & survival · joint model vs. the unsupervised two-step, including as survival signal strength varies
...
## Choosing $K_\text{init}$: the consensus sweep {.smaller .dense-cols .tight-list .compact-callout}
...
- **ELBO-preferred:** $K_\text{init}=`r elbo_pref_K`$
- **BIC-preferred:** $K_\text{init}=`r bic_pref_K`$
- **External-C-preferred:** $K_\text{init}=`r c_pref_K`$

The three do **not** agree — reported honestly, not retuned to force a match. BIC's complexity penalty (`r sprintf("~%s", format(round((K_INIT_REC+1)*(273+N_GENES_TRAIN+1) - K_INIT_REC*(273+N_GENES_TRAIN+1)), big.mark=","))` per unit $K_\text{init}$) dominates once $K_\text{init}$ climbs past single digits, while external C-index keeps rewarding a bit more capacity before plateauing.
...
::: {.callout-tip}
### Recommendation: $K_\text{init}=`r K_INIT_REC`$
Survival-active count is **stable at `r yfb_keff`** across a wide range of $K_\text{init}$ — added capacity beyond that goes to genomics-only or dead factors, ARD working as intended. $K_\text{init}=`r K_INIT_REC`$ is kept as the practical default: reachable from a single fresh fit, externally competitive, and parsimonious ($K_\text{eff}=`r K_EFF_TOTAL_REC`$ kept of `r K_INIT_REC`).
:::
...
::: {.notes}
Here is where the disagreement matters. ELBO and BIC agree with each other but
pull toward a smaller K_init than external C-index does. I'm not going to
pretend they agree — the honest read is that BIC's df penalty, which is on the
order of tens of thousands per unit K here, dominates once K climbs into double
digits, while external C-index keeps rewarding capacity a bit longer before
plateauing. K_init=7 is kept as the practical recommendation because the
survival-active count is stable there and beyond, and it's the simplest
single-fit path — but I want to show you the actual curves and provoke the
question of whether a different K_init in that plateau might be preferable.
:::
...
## Synthetic Results — Recovery & Held-out Performance {.smaller .paired-figures .tight-list}
...
::: {.notes}
Placement pending: the simulation now selects K via over-specified K_init + ARD
pruning instead of the June deck's oracle true-K. If ARD-based selection
produces a materially new finding relative to June, this goes in the main body;
if it essentially reproduces June under the new selection procedure, it moves to
the appendix (never cut — it's still the evidence backing the method, and
useful for Q&A).
:::

...
- **ELBO- and BIC-preferred:** $K_\text{init}=`r elbo_pref_K`$ (both favor the smallest $K_\text{init}$ tested). **External-C-preferred:** $K_\text{init}=`r c_pref_K`$ (favors more capacity).
...
![**Figure.** $K_\text{eff}$ (ARD-kept factor count) by $K_\text{init}$, split survival-active vs. genomics-only. $K_\text{eff}$ steps up from 2 at $K_\text{init}=2$ to a plateau of 5 by $K_\text{init}\approx9$, rather than staying fixed as $K_\text{init}$ grows.](assets/k_eff_step.png){fig-align="center" width="100%"}
...
(K_init=11, 13) show a visible external-C dip against their neighbors. The bar
chart on the right makes the same K_eff column visual: a step-up from 2 to a
plateau of 5 as K_init grows past ~9, driven mostly by additional
genomics-only factors, not additional survival-active ones -- the
survival-active count itself stays at 2-3 throughout.
:::

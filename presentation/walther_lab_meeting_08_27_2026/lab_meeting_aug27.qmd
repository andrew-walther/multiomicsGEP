...
![**Figure.** Gene-weight heatmap (top 30 genes) for the recommended model: projection $(YF)\beta$, per-platform z-standardization, survival-ranked gene selection (`r N_GENES_TRAIN` genes), no cohort indicator, $K_\text{init}=`r K_INIT_REC`$. Weights are **non-negative** (point-exponential prior). Survival-active programs ($|\hat\beta|>10^{-3}$) are starred.](assets/real_gep_heatmap.png){fig-align="center" width="62%"}
...
Note the largest expression program — the stromal collagen block — is NOT
survival-active, exactly the separation supervision is supposed to give you.
Weights are non-negative because L and F use a point-exponential prior.
:::
...
*(Two-arm external C-index figure — projection $(YF)\beta$ vs. EBMF → Cox — inserted here, Phase 4.)*
...
cat(sprintf(
  ...
  yfb_mean,
  if (!is.na(ebmf_mean)) sprintf(" · unsupervised EBMF → Cox %.3f", ebmf_mean) else ""))
...
::: {.notes}
The real-data analog of the synthetic slide, once the two-arm figure lands. Two
arms: the recommended projection model, and the unsupervised EBMF-to-Cox
two-step baseline.
:::

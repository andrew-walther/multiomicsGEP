...
updates), and a per-block KL term that is the adaptive shrinkage. In the recommended projection
model, the \key{shared $F$ enters both likelihood terms --- that coupling is the supervision}: $F$
shapes expression reconstruction ($Y \approx LF^\top$) and simultaneously shapes the survival risk
score ($\eta_i = (YF)_i\beta$). And it balances the two automatically: \key{no $\alpha$ to tune}
($\lambda = 1$).

\begin{itemize}
\item \sectionlabel{Main Takeaway:} Supervision is built into a single joint objective; the shared
$F$ in both likelihood terms is the mechanism (in the projection model).
...
\item \sectionlabel{Good Line to Stress:} ``Both likelihoods share the same gene loading matrix, so
the factors cannot specialize to only one task.''

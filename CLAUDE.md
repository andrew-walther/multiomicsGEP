...
- **Tests:** Run `Rscript tests/run_tests.R` after any change to a modular update script. Expected: 443/443 passing.
...
| Joint log-likelihood / BIC for K_init sweeps | `code/compute_bic.R` — `compute_joint_ll_bic()`; source after `fit_cox_on_yf.R`. Documents its own 3 in-sample limitations (event-count vs. n, df by K_init not K_eff, genomics-scale penalty mismatch) |
| **Cross-validated held-out log-likelihood** | `code/compute_cv_loglik.R` — `cv_survival_loglik()` (held-out Cox partial LL, 5-fold CV, cohort-aware) and `bicv_genomics_loglik()` (bi-cross-validated genomics LL); companion to `compute_bic.R`, reported separately, never combined into a "CV-BIC" |
| **Cohort-specific survival coefficients** | `code/update_beta_cohort.R` — `beta_cohort_id` argument on `fit_cox_on_yf()`; `update_beta_cohort_k()`/`_all()` (partial-pooling EBNM across cohorts), `compute_pooled_beta()` (external-prediction fallback). Distinct from `cohort_id` (genomics offset) and `strata_id` (baseline hazard) — see DECISIONS.md 2026-09-04 |
...
| **K_init consensus sweep** | `results/benchmark_sim/run_k_init_sweep.R` — K_init=2..20, ELBO/BIC/in-sample LL + (since 2026-09-04) held-out survival LL + bi-CV genomics LL + external C, `--reuse-cache`/`--reuse-cv-cache` |
| **K_init multistart dip check** | `results/benchmark_sim/run_k_init_multistart_dip_check.R` — 15-restart best-ELBO check at K_init 10-15; tests whether the K_init=11/13 external-C dip is a single-init artifact (it isn't — DECISIONS.md 2026-09-04) |
| **Threshold sensitivity** | `results/benchmark_sim/run_threshold_sensitivity.R` — `beta_thresh`/`pve_thresh` grid, `rel_thresh` sweep, beta comparability check, all from cached K_init sweep fits (no re-fitting) |
| **Cohort-aware model comparison (Stage 3)** | `results/benchmark_sim/run_cohort_beta_comparison.R` (5-arm external-C table), `run_cohort_beta_bootstrap_ci.R` (paired CIs vs. baseline), `run_cohort_beta_leave_one_out.R`, `run_cohort_beta_supplementary.R` (`strata_only` sub-arm + held-out survival LL per arm), `run_training_set_subanalysis.R` (does pooling TCGA_PAAD+CPTAC help? — yes) |
| **Factor comparison (supervised vs. unsupervised)** | `results/benchmark_sim/run_factor_comparison.R` — ARD-kept programs vs. EBMF K=40 factors, gene-list overlap + variance rank, no performance metric |
| **Multi-cohort simulation (shared/specific factor recovery)** | `results/multi_cohort_sim/run_multicohort_sim.R` — K_init 2/3/4 (under-specified) + 6/12/20 (oracle/over-specified), 15 seeds; `run_cohort_beta_recovery_sim.R` (ground-truth recovery test for `beta_cohort_id`); `run_top2_match_diagnostic.R` (per-estimated-factor top-2 true-factor correlation, the specific "do factors merge" check); `make_delta_fig.R` |
| Test suite (core + predict) | `tests/run_tests.R` (443/443) |

...
- **Current model status:** Both LB (`code/fit_modular.R`, η = Lβ) and YFB (`code/fit_cox_on_yf.R`, η = (YF)β) are fully implemented with training concordance sign correction. Both support a `cohort_id` parameter (corner-point encoding) to absorb platform offsets. **Recommended configuration (DeSurv-aligned, D4):** YFB × per-platform z-std × DeSurv gene selection (combined_rank, top-3000 per cohort before normalization) × no cohort indicator, K=7 (1-SE CV), **mean external C=0.642 across 5 held-out PDAC cohorts, K_eff=4** (post-Phase-1: objective normalization + λ retirement + train/test preprocessing fix, `DECISIONS.md` 2026-07-12; supersedes the pre-Phase-1 C=0.636/K_eff=2 figure, `DECISIONS.md` 2026-05-27). DeSurv gene selection yields 2064 genes vs 2000; delta_yfb = +0.012 (pre-Phase-1 baseline). Per-platform z-standardization remains essential: 10 of 12 non-per-platform configurations collapse to β=0 in the 18-config extended benchmark. Any preprocessing that does not normalize per-platform before merging (joint z-std, log-only, or joint QN without rank transform) collapses β→0 for both LB and YFB. **Open item:** K_eff rose from 2 to 4 alongside the C-index gain — the two newly-active factors (β≈0.003–0.004) are only marginally above `beta_threshold=0.001`, while the original two (β≈0.019, −0.042) remain dominant and consistent in sign/magnitude with the pre-Phase-1 result; `beta_threshold` was calibrated against the old (unboosted) β precision scale and likely needs revisiting now that Phase 1a increases β's absolute precision — see `ROADMAP.md`. See `ROADMAP.md` for open items.
...
| Test suite (core + predict) | `tests/run_tests.R` (246/246) |

...
$$Y \approx LF^\top \quad (n \times p \text{ matrix, } L \text{ is } n \times K,\, F \text{ is } p \times K)$$

with feature-wise precision $\tau_j$ (heteroscedastic noise). The residual for factor k at feature j is:
...
### EBNM Sufficient Statistics for $\mathbf{f}_k$
...
$$\widetilde{L}_k^{(2)} = \sum_i \mathbb{E}[l_{ik}^2] \quad \text{(scalar; includes } \text{Var}_q(l_{ik}) + \bar{l}_{ik}^2\text{)}$$
...
The $\tau_j$ factors in numerator and denominator cancel exactly. The EBNM observation $x_j$ is **identical regardless of feature precision**.
...
Here $\tau_j$ remains under the square root. A feature with higher precision (larger $\tau_j$) gets a **smaller** noise level $s_j$, which means the prior applies less shrinkage and the posterior mean stays closer to $x_j$.

### Consequence

Features with the same residual signal $x_j$ are differentially shrunk toward zero depending on their noise precision. Clean features (high $\tau_j$) are shrunk less; noisy features (low $\tau_j$) are shrunk more. This is the heteroscedastic shrinkage mechanism for F.

| $\tau_j$ | $x_j$ | $s_j$ | Shrinkage |
|----------|--------|--------|-----------|
| 1 | same | large | strong |
...
Computes the variational posterior for a single factor column $\mathbf{f}_k$.
...
**Purpose:** Demonstrate the core mathematical property that $\tau_j$ cancels in $x_j$ but not in $s_j$.
...
$\sum_i \mathbb{E}[l_{ik}^2]$ is computed once (a single scalar) and broadcast across all p features via element-wise multiplication with $\tau_j$. This avoids a p × n matrix multiply for $A_F$.

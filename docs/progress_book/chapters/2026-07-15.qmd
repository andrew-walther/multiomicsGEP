...
- **Objective cleanup.** Dropped the redundant $\lambda$ mixing weight between the genomics and
  survival halves of the objective — $\alpha$ already does that job, so $\lambda$ was dead weight.
  ...
  $\lambda$ to actually trade off. Genuinely useful, separately: caught a **train/test preprocessing
  ...
  earlier 0.636 to **`r rec_mean`**. That's a correction, not a regression.
- **Optional per-study baseline hazard.** Added a `strata(study)` option to the Cox partial
  likelihood, so each cohort can have its own baseline hazard instead of sharing one. Tested it —
  performance-neutral (0.6267 to 0.6263) — so it's available but left off by default.
...
(both near chance, as they should be — there's nothing to detect). As the scaling factor increases,
the joint model pulls ahead, in 3 of 4 simulation designs we tried. The fourth design (loadings that
...
**Shared vs. cohort-specific prognostic factors** — same simulation framework, varying how much of
...
When nothing is shared, the two approaches tie — expected, since there's no cross-cohort structure
...
outright on 2 of 5 seeds, dragging its mean down; the joint model stays stable across all 5 and also
...
doing something narrower and more defensible than the original pitch, and the numbers now hold up
under scrutiny from multiple independent angles instead of just the one fit we happened to run.
Framing-wise, SBMF reads best as the **Bayesian counterpart to DeSurv** — same modeling setup and
scoring, our version adds shrinkage priors, posterior uncertainty, and a per-gene noise model — not
as a claim that we already beat DeSurv on their own turf. A matched, same-protocol comparison
against DeSurv directly is the natural next step to make that claim (or not).

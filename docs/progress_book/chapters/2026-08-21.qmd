...
**Factor biology is still preliminary (§2.5).** Top-20 gene lists for both survival-active programs
are included for direct inspection, along with the gene-weight heatmap for all 4 kept factors
(§3.4). The methods used so far were convenience picks and need discussion before any biological
label is treated as settled.

**Largest outstanding item.** The matched, same-protocol head-to-head against DeSurv (carried from
8/3) has not been started. Everything to date compares against a two-step baseline or DeSurv's
published gene programs, not DeSurv's own predictive performance under our protocol.
...
- **Matched, same-protocol head-to-head against DeSurv** (carried from 8/3, not started this cycle).
  Same data, same scoring, as a performance claim rather than the biology-convergence claim the
  8/3 gene-overlap comparison already supports. This is the largest outstanding methodological item.
...
- **Firming up the factor-biology methods** (§2.5). The enrichment and subtype checks run so far were
  convenience picks and need discussion before any biological label is treated as settled.
- **Manuscript introduction and background/literature review**, continuing from the 8/3 handoff.
...
**Preliminary biology:** Treat this as a placeholder to
revisit once we're aligned on which factors matter, not as settled biology:
...
- Program 3's list doesn't jump out the way Program 7's does; it needs the
  formal methods below rather than a read-off.
...
**Preliminary Factor Biology Methods:** The pathway-enrichment and subtype checks
already run on Programs 3/7 (gene-set enrichment via fgsea/ORA against curated PDAC gene sets,
correlation with PurIST classical/basal subtype scores, overlap with DeSurv's own published gene
programs, and consistency of the hazard-ratio direction across the 5 external cohorts) were picked
for convenience as an initial look into the factor biology. Further discussion & direction for understanding factor biology is necessary.

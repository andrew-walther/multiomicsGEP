...
- *Increasing survival signal from zero:* at zero signal, joint $\approx$ two-step
  (both near chance); as signal grows, the joint model exceeds the two-step
  baseline by a growing margin, in 3 of 4 data-generating designs. One artificial design (exactly-sparse,
...
```
  ...
  collapses on 2 of 5 seeds (which lowers its mean to 0.717), while the joint model is

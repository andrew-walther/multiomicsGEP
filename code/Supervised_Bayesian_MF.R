# ------------------------------------------------------------------------------
# TITLE:       Supervised Bayesian Matrix Factorization (Review & Validation)
...
#   outcomes (t, delta). The code uses Variational Inference to find a shared 
#   latent space (L) that minimizes reconstruction error and hazard residuals.
# ------------------------------------------------------------------------------
...
library(survival)
# 'ebnm' provides Empirical Bayes Normal Means solvers, estimating sparse 
# Point-Normal priors for L, F, and Beta.
library(ebnm) 
...
#' into a locally linear "Weighted Least Squares" form via Taylor expansion.
#' This is the "bridge" that allows survival data to pull on the latent loadings.
calc_cox_taylor <- function(eta, time, status) {
  n <- length(time)
  # Sorting is required to calculate risk sets in Cox partial likelihoods.
  ord <- order(time) 
  ...
  # A numerical floor prevents division-by-zero for low-information patients.
...
#' 
#' Calculates Harrell's Concordance Index (C-index) to prove that the 
#' Supervised Latent space is a superior predictor compared to PCA.
...
#' 
#' Ranks genomics features (row indices) by their weight in the Factor matrix (F).
#' This identifies the "Key Regulatory Sites" defining each biological program.
get_top_features <- function(EF, n_top = 10) {
  lapply(1:ncol(EF), function(k) {
    weights <- EF[,k]
    order_idx <- order(abs(weights), decreasing = TRUE)
    data.frame(FeatureID = order_idx[1:n_top], Weight = round(weights[order_idx[1:n_top]], 4))
  })
}
...
#' of Variance Explained (PVE) into a single summary view.
get_factor_summary_table <- function(res, data) {
  ...
  data.frame(Factor = 1:K, Beta = round(res$Beta, 3), LogRank_P = round(unlist(p_vals), 4),
             Sparsity_Pct = round(sparsity, 2), PVE_Pct = round(pve, 2))
}
...
fit_supervised_mf <- function(Y, time, status, K = 5, max_iter = 100, tol = 1e-5) {
  ...
  # Initialization via SVD provides aDeterministic and variance-maximized starting subspace.
  svd_init <- svd(Y, nu = K, nv = K)
  EL <- svd_init$u %*% diag(sqrt(svd_init$d[1:K]), K, K) 
  EF <- svd_init$v %*% diag(sqrt(svd_init$d[1:K]), K, K)
  EL2 <- EL^2; EF2 <- EF^2
  
  # Warm-start Beta via Cox on initial loadings ensuring early clinical signal recognition.
  ...
  for(iter in 1:max_iter) {
    ...
    # 2. Coordinate Ascent Updates (Sequential Factor Optimization)
    for(k in 1:K) {
      # --- Update L (Fusion Step) ---
      # Combines precision from genomics and survival relevance.
      ...
      EL2[,k] <- res_L$posterior$sd^2 + res_L$posterior$mean^2 # Incorporate variance
      ...
      # --- Update Beta (Clinical Weight) ---
      # Incorporation of EL2 implements the "Uncertainty Penalty."
      A_Beta <- sum(w * EL2[,k]) 
    ...
    # 3. Periodic Orthogonalization: Ensures biological programs remain distinct.
    if(iter %% 10 == 0) {
    ...
    # 4. Precision Update (Tau): Feature-specific measurement noise.
...
plot_gep_heatmap <- function(res, n_features = 50) {
  ...
  par(mar=c(6, 4, 4, 1))
  image(1:nrow(F_sub), 1:ncol(F_sub), F_sub, main=paste("GEP Feature Weights (Top", n_features, "IDs)"),
        xlab="Feature ID (Row Index)", ylab="Latent Factors", col=palette, axes=FALSE, zlim=c(-max_val, max_val))
  ...
  image(1, seq(-max_val, max_val, length.out=100), t(legend_image), col=palette, axes=FALSE, xlab="", ylab="")
  axis(4, las=1, cex.axis=0.8); mtext("Weight", side=4, line=2, cex=0.8); layout(1)
}

#' Diagnostic Dashboard
#' 
#' Renders four independent panes into RStudio to evaluate clinical and genomic fit.
visualize_dashboard <- function(res, data) {
  std_mar <- c(5, 5, 4, 2)
  # Pane 1: Trace
  par(mfrow=c(1,1), mar=std_mar)
  plot(res$history$rmse[res$history$rmse > 0], type="l", lwd=2, col="#1f77b4", main="Optimization Trace", xlab="Iteration", ylab="RMSE", bty="n")
  abline(h = 1.0, col = "red", lty = 2); grid(col="lightgray", lty="dotted")
  # Pane 2: Identity
  plot_gep_heatmap(res)
  # Pane 3: Clinical Stratification
  ...
  for(k in 1:K) {
    ...
    plot(km, col=c("#d62728", "#1f77b4"), lwd=2, bty="n", main=paste("Factor", k, "\n", p_label), xlab="Time", ylab="Prob")
  }
  # Pane 4: Verification
  if(!is.null(data$L_true)) {
    ...
    plot(data$L_true[,target_true], res$L[,target_est] * sign_correction, main=paste("Signal Recovery (Matched: Est", target_est, "vs True", target_true, ")"), 
         xlab="Ground Truth Signal", ylab="Model Estimated (Sign Corrected)", col=rgb(0,0,0,0.4), pch=16, bty="n")
...
if(DATA_MODE == "simulated") {
  # JUSTIFICATION FOR SYNTHETIC PARAMETERS:
  # - N=250: Simulates a standard clinical cohort (e.g. TCGA subset).
  # - P=1000: Represents pre-filtered high-variance genomic features.
  # - K=5: Complex enough for overlapping signals, small enough for interpretation.
  ...
  for(k in 1:length(top_feats)) { cat(sprintf("GEP %d:\n", k)); print(top_feats[[k]]) }
